...
5. Practice participation must be exact.
   ...
   Practice information is optional. If you cannot provide a
   specific valid source URL supporting the practice information,
   return null for ALL latest_practice_* fields. Do not return an
   unsourced practice claim.
...
7. newest_credible_update should represent the freshest meaningful
   ...
   This update is optional. If you cannot provide a specific valid
   source URL whose publisher matches the hosting organization,
   return null for ALL newest_credible_update_* fields. Do not
   return an update with an unverified or mismatched source.
...
def sanitize_optional_practice_sources(
    research_report,
):
    """
    Remove optional practice information when its supporting source
    fails deterministic validation.

    Practice information is useful, but it must never be allowed to
    fail the entire unattended weekly report solely because the model
    returned an unsourced or invalid optional practice claim.

    This remains fail-closed for the fact itself: invalid practice
    information is discarded rather than accepted.
    """

    omitted_errors = []

    for player in research_report.players:
        has_any_practice_data = any(
            [
                player.latest_practice_date,
                player.latest_practice_participation,
                player.latest_practice_source_publisher,
                player.latest_practice_source_url,
            ]
        )

        if not has_any_practice_data:
            continue

        # A date/source without an actual participation fact is not
        # useful to downstream lineup logic. Keep the practice block
        # internally consistent by clearing it.
        if not player.latest_practice_participation:
            player.latest_practice_date = None
            player.latest_practice_source_publisher = None
            player.latest_practice_source_url = None
            continue
        ...
        omitted_errors.extend(source_errors)

        omission_note = (
            "Practice information was omitted because its supporting "
            "source did not pass deterministic validation."
        )

        if player.notes:
            if omission_note not in player.notes:
                player.notes = (
                    player.notes.rstrip()
                    + " "
                    + omission_note
            ...
            player.notes = omission_note

        player.latest_practice_date = None
        player.latest_practice_participation = None
        player.latest_practice_source_publisher = None
        player.latest_practice_source_url = None

    return omitted_errors


def sanitize_optional_newest_update_sources(
    research_report,
):
    """
    Remove an optional newest-credible-update block when its source
    fails deterministic validation.

    The newest update is supplemental context. Invalid or mismatched
    sourcing must never be accepted, but it also should not cancel an
    otherwise source-valid unattended weekly report.

    Core injury facts and official game status remain subject to the
    normal strict validator.
    """

    omitted_errors = []

    for player in research_report.players:
        has_any_update_data = any(
            [
                player.newest_credible_update_datetime,
                player.newest_credible_update_publisher,
                player.newest_credible_update_url,
                player.newest_credible_update_summary,
            ]
        )

        if not has_any_update_data:
            continue

        # Metadata without an actual update summary is not useful.
        # Clear the entire optional block to keep it internally
        # consistent.
        if not player.newest_credible_update_summary:
            player.newest_credible_update_datetime = None
            player.newest_credible_update_publisher = None
            player.newest_credible_update_url = None
            continue
        ...
        omission_note = (
            "Newest credible update was omitted because its "
            "supporting source did not pass deterministic validation."
        )
        ...
        player.newest_credible_update_publisher = None
        player.newest_credible_update_url = None
        player.newest_credible_update_summary = None
...
If practice information cannot be supported by a specific valid
source URL, return null for all latest_practice_* fields.
If the newest credible update cannot be supported by a specific
valid source URL whose publisher matches the hosting organization,
return null for all newest_credible_update_* fields.
"""
    ...
    for attempt in range(
        ...
        # Practice information is optional. If the model returns a
        # practice claim without a deterministically valid supporting
        # source, discard that practice block instead of allowing the
        # optional fact to cancel the entire unattended weekly report.
        #
        # This remains fail-closed: the unsourced fact is NOT accepted.
        sanitize_optional_practice_sources(
            research_report
        )

        # The newest credible update is also supplemental context.
        # Reject a bad source for that fact by omitting the optional
        # update instead of cancelling the entire weekly report.
        sanitize_optional_newest_update_sources(
            research_report
        )
